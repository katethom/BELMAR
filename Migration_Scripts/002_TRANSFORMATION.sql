...
select * into clients_clean from (
	select 
		...
		, contacttype
		, status
		, CONVERT(varchar, cast(assignmentdate as date), 23) as assignmentdate
...
drop table if exists contact_details_clean

select * into contact_details_clean from(
	select 
		null as accountid
		, clientid as clientid_src
		, contactvalue as workemail
		, null as workphone
		, null as email
		, null as workphone2
		, null as homephone 
	from contactdetails where contacttype = 'Work Email'
union
	select 
		null as accountid
		, clientid as clientid_src
		, null as workemail
		, contactvalue as workphone
		, null as email
		, null as workphone2
		, null as homephone 
	from contactdetails where contacttype = 'Work Phone'
union
	select 
		null as accountid
		, clientid as clientid_src
		, null as workemail
		, null as workphone
		, contactvalue as email
		, null as workphone2
		, null as homephone 
	from contactdetails where contacttype = 'Email'
union
	select 
		null as accountid
		, clientid as clientid_src
		...
		, null as workphone
		, null as email
		, contactvalue as workphone2
		, null as homephone 
	from contactdetails where contacttype = 'Work Phone2'
union
	select 
		null as accountid
		, clientid as clientid_src
		...
		, null as email
		, null as workphone2
		, contactvalue as homephone 
	from contactdetails where contacttype = 'Home Phone'
) as contact_details_clean

--select count(distinct clientid_src) from contact_details_clean

drop table if exists contact_details_flat

select 
accountid
, clientid_src
, max(workemail) as workemail
, max(workphone) as workphone
, max(email) as email
, max(workphone2) as workphone2
...
select * 
into business_clients 
from business_with_details where contacttype = 'Business'
